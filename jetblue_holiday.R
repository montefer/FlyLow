...
dir.create("Holiday_Flights_To_From_Plots")
...
generate_holiday_plot<-function(depart, arrive){
  ...
  ad_df<-tail(head(ad_df,nrow(ad_df)/3),nrow(ad_df)/4)
  ...
  plot<-plot +
    labs(title = paste("Holiday Price vs. Date from",depart,"to",arrive), x = "Date", y = "Price") +
    theme(axis.text.x=element_text(angle=40, size=10, vjust=0.5))
  
  
  #Save plot  
  ggsave(filename = paste("Holiday_Flights_To_From_Plots/",depart," to ",arrive,".png", sep=""),plot = last_plot())
